...
public:
    ...
    T dot(const Vector2& v1) const {
        return Tuple2<T>::x*v1.x + Tuple2<T>::y*v1.y;
    }
    ...
    T lengthSquared() const {
        return Tuple2<T>::x*Tuple2<T>::x + Tuple2<T>::y*Tuple2<T>::y;
    }
    ...
    T angle(const Vector2& v1) const {
        // stabler than acos
        return VmUtil<T>::abs(VmUtil<T>::atan2(Tuple2<T>::x*v1.y - Tuple2<T>::y*v1.x , dot(v1)));
    }
